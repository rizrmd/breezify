<div x-init="$wire.checkUpdate" x-data="upgradeModal({
        currentVersion: @js($currentVersion),
        latestVersion: @js($latestVersion),
        devMode: @js($devMode),
        isUpgradeAvailable: @js($isUpgradeAvailable)
    })">
    <div :class="{ 'z-40': modalOpen }" class="relative w-auto h-auto">
        <button class="menu-item cursor-pointer" @click="checkAndOpen" :title="isUpgradeAvailable ? 'New version available' : 'Click to check for updates'">
            <svg xmlns="http://www.w3.org/2000/svg"
                class="w-6 h-6 transition-colors hover:text-pink-300"
                :class="isUpgradeAvailable ? 'text-pink-500' : 'text-neutral-500 dark:text-neutral-400'"
            ...
            <span class="menu-item-label" x-text="isUpgradeAvailable ? 'Upgrade' : 'Check Updates'"></span>
        </button>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('upgradeModal', (config) => ({
            ...
            isUpgradeAvailable: config.isUpgradeAvailable || false,

            checkAndOpen() {
                this.$wire.checkUpdate().then(() => {
                    this.isUpgradeAvailable = this.$wire.isUpgradeAvailable;
                    this.latestVersion = this.$wire.latestVersion;
                    this.currentVersion = this.$wire.currentVersion;
