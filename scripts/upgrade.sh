...
CDN="https://raw.githubusercontent.com/rizrmd/breezify/v4.x"
...
HELPER_IMAGE="${REGISTRY_URL:-ghcr.io}/rizrmd/breezify-helper:${LATEST_HELPER_VERSION}"
...
nohup bash -c "
    ...
    if [ -f /data/coolify/source/docker-compose.custom.yml ]; then
        ...
        docker run -v /data/coolify/source:/data/coolify/source -v /var/run/docker.sock:/var/run/docker.sock \${DOCKER_CONFIG_MOUNT} --rm \${REGISTRY_URL:-ghcr.io}/rizrmd/breezify-helper:\${LATEST_HELPER_VERSION} bash -c 'LATEST_IMAGE=\${LATEST_IMAGE} docker compose --env-file /data/coolify/source/.env -f /data/coolify/source/docker-compose.yml -f /data/coolify/source/docker-compose.prod.yml -f /data/coolify/source/docker-compose.custom.yml up -d --remove-orphans --wait --wait-timeout 60' >>"\$LOGFILE" 2>&1
    else
        log 'Using standard docker-compose configuration'
        log 'Running docker compose up...'
        docker run -v /data/coolify/source:/data/coolify/source -v /var/run/docker.sock:/var/run/docker.sock \${DOCKER_CONFIG_MOUNT} --rm \${REGISTRY_URL:-ghcr.io}/rizrmd/breezify-helper:\${LATEST_HELPER_VERSION} bash -c 'LATEST_IMAGE=\${LATEST_IMAGE} docker compose --env-file /data/coolify/source/.env -f /data/coolify/source/docker-compose.yml -f /data/coolify/source/docker-compose.prod.yml up -d --remove-orphans --wait --wait-timeout 60' >>"\$LOGFILE" 2>&1
    fi
